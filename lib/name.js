...
const nameChanger = async (link, nama, password) => {
    try {
        ...
        if(await (await (await page.$('#lloginfailed')).getProperty('innerHTML')).jsonValue() === 'Invalid username or password , please input the correct username or password.') return console.log('Password WEBUI bukan "admin" silahkan ganti atau reset MODEM')
